...
function run(options) {
  ...
  run.kill = function (noRestart, callback) {
    ...
    if (child !== null) {
      ...
      /* Now kill the entire subtree of processes belonging to nodangel */
...
function kill(child, signal, callback) {
  ...
  if (utils.isWindows) {
    ...
    if (hasPS) {
      // we use psTree to kill the full subtree of nodangel, because when spawning
      // processes like `coffee` under the `--debug` flag, it'll spawn it's own
      // child, and that can't be killed by nodangel, so psTree gives us an array
      // of PIDs that have spawned under nodangel, and we send each the SIGUSR2
