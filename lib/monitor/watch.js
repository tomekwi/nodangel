  ...
  if (config.system.useFind) {
    ...
    changeFunction = function (lastStarted, callback) {
      ...
      if (delta > 0) {
        ...
        touch.sync('.nodangel-find-ref', { time: new Date(Date.now() + (delta * 1000)) });
        mtime = '-newer .nodangel-find-ref';
      }

      // don't run the find when delta is 0 seconds, otherwise
      // nodangel goes into a barmy restart loop
      if (delta === 0) {
        return callback([]);
      }
      ...
      exec(cmds.join(';'), function (error, stdout) {
        ...
        if (delta > 0) {
          try { fs.unlinkSync('.nodangel-find-ref'); }
