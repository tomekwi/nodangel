...
function execFromPackage() {
  ...
  try {
    // note: this isn't nodangel's package, it's the user's cwd package
 ...
 *
 * @param  {Object} nodangelOptions
 * @param  {Object} execMap
 * @return {Object} new and updated version of nodangelOptions
 */
function exec(nodangelOptions, execMap) {
  if (!execMap) {
    execMap = {};
  }
  ...
  // first, then the user options (via nodangel.json) then finally default down to
  // pot shots at the directory via package.json
  if (!nodangelOptions.exec && !nodangelOptions.script) {
    var found = execFromPackage();
    if (found !== null) {
      if (found.exec) {
        nodangelOptions.exec = found.exec;
      }
      if (!nodangelOptions.script) {
        nodangelOptions.script = found.script;
      }
      if (Array.isArray(nodangelOptions.args) && nodangelOptions.scriptPosition === null) {
        nodangelOptions.scriptPosition = nodangelOptions.args.length;
      }
    }
  }

  var options = utils.clone(nodangelOptions || {}),
  ...
  // and 'js jade' => js,jade
  // BIG NOTE: user can't do this: nodangel -e *.js
  ...
  if (({}).toString.apply(nodangelOptions.env) === '[object Object]') {
    options.env = utils.clone(nodangelOptions.env);
  } else if (nodangelOptions.env !== undefined) {
    throw new Error('nodangel env values must be an object: { PORT: 8000 }');
  }
