...
/**
 * Load the nodangel config, first reading the global root/nodangel.json, then
 * the local nodangel.json to the exec and then overwritting using any user
...
function load(settings, options, config, callback) {
  config.loaded = [];
  // first load the root nodangel.json
  loadFile(options, config, utils.home, function (options) {
    // then load the user's local nodangel.json
    loadFile(options, config, process.cwd(), function (options) {
      ...
      */
      ...
      // if we didn't pick up a nodangel.json file & there's no cli ignores
      // then try loading an old style .nodangelignore file
      if (config.loaded.length === 0) {
        // TODO decide whether this is just confusing...
        var legacy = loadLegacyIgnore.bind(null, options, ready);

        // first try .nodangelignore, if that doesn't exist, try nodangel-ignore
        return legacy('.nodangelignore', function () {
          legacy('nodangel-ignore', function (options) {
            ready(options);
          });
        });
      }
...
/**
 * Loads the old style nodangelignore files which are simply a list of patterns
 * in a file to ignore
 *
 * @param  {Object} options    nodangel user options
 * @param  {Function} success
 * @param  {String} filename   the ignore file (.nodangelignore or nodangel-ignore)
...
function loadFile(options, config, dir, ready) {
  ...
  var callback = function (settings) {
    // prefer the local nodangel.json and fill in missing items using
    // the global options
  ...
  var filename = path.join(dir, 'nodangel.json');
