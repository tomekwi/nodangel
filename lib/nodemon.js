...
function nodangel(settings) {
  nodangel.reset();

  // allow the cli string as the argument to nodangel, and allow for
  // `node nodangel -V app.js` or just `-V app.js`
  if (typeof settings === 'string') {
    settings = settings.trim();
    if (settings.indexOf('node') !== 0) {
      if (settings.indexOf('nodangel') !== 0) {
        settings = 'nodangel ' + settings;
      }
  ...
  // nodangel tools like grunt-nodangel. This affects where
  // the script is being run from, and will affect where
  // nodangel looks for the nodangel.json files
  if (settings.cwd) {
  ...
  config.load(settings, function (config) {
    ...
    if (config.options.restartable) {
      // allow nodangel to restart when the use types 'rs\n'
      ...
      // if 'restartable' is disabled (via a nodangel.json)
    ...
    if (config.options.dump) {
      ...
      utils.log._log('log', 'nodangel: ' + version);
      ...
      if (config.options.stdout === false) {
        bus.on('start', function() {
          nodangel.stdout = bus.stdout;
          nodangel.stderr = bus.stderr;
...
nodangel.restart = function () {
  utils.log.status('restarting child process');
  bus.emit('restart');
  return nodangel;
};

nodangel.addListener = nodangel.on = function (event, handler) {
...
nodangel.once = function (event, handler) {
  if (!eventHandlers[event]) { eventHandlers[event] = []; }
  eventHandlers[event].push(handler);
...
nodangel.emit = function () {
  bus.emit.apply(bus, [].slice.call(arguments));
  return nodangel;
};

nodangel.removeAllListeners = function (event) {
  // unbind only the `nodangel.on` event handlers
...
nodangel.reset = function (done) {
  monitor.run.kill(true, function () {
    nodangel.removeAllListeners();
    utils.reset();
    config.reset();
    config.run = false;
    if (done) {
...
nodangel.config = config;

// on exception *inside* nodangel, shutdown wrapped node app
if (!config.required) {
  process.on('uncaughtException', function (err) {
    console.error('exception in nodangel killing node');
